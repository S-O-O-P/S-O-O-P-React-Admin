...
export default function EventsInfoApi({setEvents}, apiName, earlyCode = null, eventData = null){
  ...
  const selectDetailEventsInfo = (earlyCode) => {
    axios.get('http://localhost:8080/cultureinfo/early/'+earlyCode)
      .then(response => {
  ...
  const registerNewEventsInfo = (eventData) => {
    axios.post('http://localhost:8080/cultureinfo/early', eventData)
    .then(response => {
  ...
  const editEventsInfo = (earlyCode, eventData) => {
    axios.put('http://localhost:8080/cultureinfo/early/'+earlyCode, eventData)
    .then(response => {
  ...
  const deleteEventsInfo = (earlyCode) => {
    axios.delete('http://localhost:8080/cultureinfo/early/'+earlyCode)
    .then(response => {
